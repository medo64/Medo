...
[Obsolete("Use generic WeightedMovingAverage instead (e.g. WeightedMovingAverage<double>).")]
#endif
...
/// </code>
/// </example>
public sealed class WeightedMovingAverage<T> where T : IFloatingPoint<T> {
    ...
    /// <param name="collection">Collection.</param>
    /// <exception cref="ArgumentNullException">Collection cannot be null.</exception>
    public WeightedMovingAverage(IEnumerable<T> collection)
        : this(10, collection) {
    }
    ...
    /// <exception cref="ArgumentOutOfRangeException">Count must be larger than 0.</exception>
    /// <exception cref="ArgumentNullException">Collection cannot be null.</exception>
    public WeightedMovingAverage(int subsetSize, IEnumerable<T> collection)
        : this(subsetSize) {
        AddRange(collection);
    }
    ...
    public T Average {
        get {
            if (_items.Count > 0) {
                ...
                int i = 0;
                foreach (var value in _items) {
                    var next = (T)Convert.ChangeType(i + 1, typeof(T));
                    sum += value * next;
                    divider += next;
                    i += 1;
                }
