...
namespace Medo.Security.Cryptography {
    ...
    public sealed class Pbkdf2 {
        ...
        public Pbkdf2(HMAC algorithm, string password, string salt, int iterations) :
            this(algorithm,
                 Encoding.UTF8.GetBytes(password ?? throw new ArgumentNullException(nameof(password), "Password cannot be null.")),
                 Encoding.UTF8.GetBytes(salt) ?? throw new ArgumentNullException(nameof(salt), "Salt cannot be null."),
                 iterations) {
        }
